...
for crowd dynamics have been develop. One of the first model was developed by Hirai 
and Tarui (1975) introducing the class of \textit{force based} models 
\cite{Hirai}. These models represents the interaction between 
pedestrians and their environment through different kinds of forces. For 
example, Okazaki (1979) simulated the movement of each pedestrian by the motion 
of a magnetized object in magnetic field which exerts attractive force on the 
different kind of magnetic poles \cite{Okazaki}.}\\
...
\textcolor{blue}{By the late '90s and the beginning of the century, Helbing and 
co-workers postulated that both the environment and the individuals' own desire 
affect the pedestrians motion in a similar way as forces do with respect to the 
...
Model \textcolor{red}{\cite{Helbing1}} is incapable of reproducing the 
...
middle corresponds to the measurement region.}
\end{figure}
...
$\vec{v}_d^{~(i)}=1$~m/s~$\hat{e}_d^{~(i)}$, where the target $\hat{e}_d^{~(i)}$ 
...
(\textcolor{red}{see Fig.~\ref{pasillo}}). This allowed the pedestrians to 
...
previous works (see Refs. \cite{sticco,Dorso5}), except for the friction 
coefficient $\kappa$.} \\
...
\textcolor{blue}{(named compute cluster-atom)}}.\\
...
\textcolor{red}{desired speed} limited to $v_d=1\,$m/s). \\
...
As our desired velocity is fixed at $v_d = 1$~m/s, we conclude that the speed at 
the maximum density has to be bounded by $v(\rho_{max}) \lesssim  0.5$~m/s in 
